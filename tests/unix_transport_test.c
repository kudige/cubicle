...
int main(void)
{
    ...
    int length = snprintf(socket_path, sizeof(socket_path), "%s/manager.sock",
                          directory);
    ...
    if (server_pid == 0) {
        close(ready_pipe[0]);
        run_server(socket_path, ready_pipe[1]);
    }

    close(ready_pipe[1]);
    char ready = 0;
    assert(read_all(ready_pipe[0], &ready, 1) == 0);
    assert(ready == 'R');
    close(ready_pipe[0]);
    ...
    strcpy(endpoint.address, socket_path);
    ...
    assert(transport->vtable->request(transport, "ping", 4, &response,
                                      &response_length, &error) == CUBICLE_OK);

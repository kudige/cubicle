...
static cubicle_error_code_t set_error(cubicle_client_t *client, cubicle_error_code_t code, int system_errno, const char *message)
{
    if (client != NULL) {
        client->last_error.code = code;
        client->last_error.system_errno = system_errno;
        client->last_error.retryable = false;
        snprintf(client->last_error.message, sizeof(client->last_error.message), "%s", message == NULL ? "" : message);
    }
    return code;
}

cubicle_error_code_t cubicle_client_connect(const cubicle_client_options_t *options, cubicle_client_t **client_out)
{
    if (options == NULL || client_out == NULL || options->transport == NULL || options->transport->vtable == NULL || options->transport->vtable->connect == NULL)
        return CUBICLE_ERR_INVALID_ARGUMENT;
    cubicle_client_t *client = calloc(1, sizeof(*client));
    if (client == NULL) return CUBICLE_ERR_INTERNAL;
    ...
    cubicle_error_code_t result = client->transport->vtable->connect(client->transport, &client->endpoint, &client->last_error);
    if (result != CUBICLE_OK) { free(client); return result; }
    *client_out = client;
    return CUBICLE_OK;
}
...
const cubicle_error_t *cubicle_client_last_error(const cubicle_client_t *client) { return client == NULL ? NULL : &client->last_error; }

static cubicle_error_code_t rpc_not_implemented(cubicle_client_t *client, const char *method)
{
...
cubicle_error_code_t cubicle_client_session_info(const cubicle_client_t *client, cubicle_session_info_t *session_out)
{
...
{
    if (client == NULL || options == NULL || options->name == NULL || options->name[0] == '\0' || workspace_out == NULL) return CUBICLE_ERR_INVALID_ARGUMENT;
...
{
    if (client == NULL || options == NULL || process_out == NULL || options->workspace_id == NULL || options->argv == NULL || options->argc == 0) return CUBICLE_ERR_INVALID_ARGUMENT;

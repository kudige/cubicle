...
static void desk_render(const desk_terminal_t *terminal)
{
    char frame[16384];
    size_t used = 0;
    ...
    int top_rows = body_rows / 2;
    int bottom_rows = body_rows - top_rows - 1;

    append_text(frame, sizeof(frame), &used, "\x1b[H\x1b[2J");
    append_text(frame, sizeof(frame), &used, "Cubicle Desk");
    append_repeat(frame, sizeof(frame), &used, ' ', cols - 29);
    append_text(frame, sizeof(frame), &used, "q quit | resize aware\r\n");
    append_repeat(frame, sizeof(frame), &used, '-', cols);
    append_text(frame, sizeof(frame), &used, "\r\n");

    for (int row = 0; row < body_rows; ++row) {
        const char *left = row == 0 ? "cube 1: shell" : "";
        const char *right = "";

        if (row == 0) {
            right = "cube 2: editor";
        } else if (row == top_rows + 1) {
            right = "cube 3: logs";
        }

        append_cell_text(frame, sizeof(frame), &used, left, left_width);
        append_text(frame, sizeof(frame), &used, "|");

        if (row == top_rows) {
            append_repeat(frame, sizeof(frame), &used, '-', right_width);
        } else {
            (void)bottom_rows;
            append_cell_text(frame, sizeof(frame), &used, right, right_width);
        }
        append_text(frame, sizeof(frame), &used, "\r\n");
    }
